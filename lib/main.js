...
(function (root, factory) {
	...
	define([
	    ...
	    'jjve',
	    'marc-record-js',
	    'json!../resources/configuration-schema.json',
	    'json!../resources/action-copy-options-schema.json',
	    'json!../resources/action-select-better-options-schema.json'
	], factory);
	...
	module.exports = factory(
	    ...
	    require('../resources/action-copy-options-schema.json'),
	    require('../resources/action-select-better-options-schema.json')
	);
    } else {
	root.mergeMarcRecordsFactory = factory(
	    ...
	    root.jjve,
	    root.MarcRecord,
	    root.mergeMarcRecordsConfigurationSchema,
	    root.mergeMarcRecordsActionCopySchema,
	    root.mergeMarcRecordsActionSelectBetterSchema
	);
    }

}(this, factory));

function factory(Object, shim_array, jjv, jjve, MarcRecord, schema_configuration, schema_action_copy, schema_action_select_better)
{
	...
	function validateDocument(schema, document)
	{
	    ...
	    var env = jjv();
	    var je = jjve(env);
	    var errors = env.validate(schema, document, {
		useDefault: true
	    });
	    
	    if (errors) {
		throw new Error('Invalid configuration: ' + JSON.stringify(je(schema, document, errors), undefined, 4));
	    } else {
		return document;
	    }
	...
	function getFieldConfiguration(tag) {

	    return Object.keys(configuration.fields).filter(function(field) {
		return new RegExp('^' + field +'$').test(tag);
	    }).reduce(function(result, field) {
		return configuration.fields[field];
	    }, undefined);
	...
	function makeSubfieldPairs(subfields1, subfields2, fn_comparator) {
		...
		if (!subfields2.every(function(subfield2) {

		    return subfields1.some(function(subfield1, index) {
			if (fn_comparator(subfield1, subfields2[index])) {
		    ...
		    throw new Error('Subfields do not match');
		}
	...
	function fieldsSimilar(field1, field2, options) {
	    ...
	    if (field1_normalized.tag !== field2_normalized.tag) {
		return false;
	    }
	    ...
	    if (field1_normalized.subfields && field2_normalized.subfields) {
		...
		if (field1_normalized.value !== field2_normalized.value) {
		    return false;
		}
	    }
	...
	function pickMissingSubfields(field_to, field_from, subfield_codes)
	{
		...
		.filter(function(subfield) {
		    return field_to.subfields.indexOf(subfield.code) < 0;
		})
	...
	function actionControlfield(record_merged, field_other, options, return_details) {
	...
	function actionCopy(record_merged, field_other, options, return_details) {
	    ...
	    options = validateDocument(schema_action_copy, typeof options === 'object' ? options : {});
		...
		if (fields_preferred.length > 0 && typeof options.transformOnInequality === 'object') {
		    ...
		    record_merged.fields.push(field_transformed);
		...
		if (field_preferred.subfields !== undefined && field_other.subfields !== undefined) {
		...
		if (arrayDifference(field_preferred_normalized.subfields, field_other_normalized.subfields, subfieldEquality).length === 0 && 
			...
			pickMissingSubfields(field_other_copy, field_preferred, options.pickMissing);
		    }
		    ...
		    if (options.pickMissing) {
			pickMissingSubfields(field_preferred, field_other, options.pickMissing);
		    }
	...
	function actionSelectBetter(record_merged, field_other, options, return_details) {

	    
	    var error_invalid_record, field_merged, field_merged_normalized, field_other_normalized, field_other_copy, subfield_pairs_sizes, subfield_pairs_scores, fn_comparator, index_better, missing_subfields_picked,
	    details = return_details ? {} : undefined,
	    fields_merged = record_merged.fields.filter(function(field) {
		return field.tag == field_other.tag;
	    });
	    
	    options = validateDocument(schema_action_select_better, typeof options === 'object' ? options : {});
	    fn_comparator = comparators.hasOwnProperty(options.comparator) ? comparators[options.comparator] : comparators.equality;
	    
	    if (fields_merged.length > 1) {
		...
		if (arrayDifference(field_merged_normalized.subfields, field_other_normalized.subfields, fn_comparator).length === 0 && arrayDifference(field_other_normalized.subfields, field_merged.subfields, fn_comparator).length > 0) {
	    ...
	    if (missing_subfields_picked === undefined && options.pickMissing) {
		pickMissingSubfields(fields_merged[0], field_other, options.pickMissing);
	    }
	...
	configuration = validateDocument(schema_configuration, configuration);
	applyPlugins();
	...
	return function(record_preferred, record_other, return_details)
	{
		...
		var result,
		config = getFieldConfiguration(field.tag);

		if (config !== undefined) {
		    if (actions.hasOwnProperty(config.action)) {

			result = actions[config.action](record_merged, field, config.options, return_details);
	    ...
	    record_merged.fields.sort(function(a, b) {
		
		function getSortIndex(tag) {
		    return TAG_SORT_INDEX.hasOwnProperty(tag) ? TAG_SORT_INDEX[tag] : parseInt(tag, 10);
		}
		
		return getSortIndex(a.tag) - getSortIndex(b.tag);
