  ...
  return function(configuration, plugins)
  {
        ...
        .forEach(function(action) {
          ...
          if (action.type === 'combine') {
            if (!isEmpty(mergedValue) || !isEmpty(otherValue)) {
              ...
              var combinedValue = padEnd(uniq(mergedParts.concat(otherParts).sort()).slice(0,4).join(''), 4, '^');
    ...
    function actionCreateFrom(record_merged, field_other, options, comparators, return_details) {
      ...
      if (options.useExisting) {
        var targetTag = options.convertTag || field_other.tag;

        fields_merged = record_merged.fields.filter(function(field) {
          return field.tag == targetTag;
        }); 
      }
      else {
        fields_merged = record_merged.fields.filter(function(field) {
          return field.tag == field_other.tag && (field.fromOther == undefined || field.fromOther == false);
        });
      }

      if (fields_merged.length > 1) {

        error_invalid_record = new Error('createFrom cannot be used if there are multiple fields of same tag.');
        error_invalid_record.name = 'InvalidRecordError';
        ...
        var field_merged = fields_merged[0];
        var field_other_copy;

        if (options.useExisting && field_merged) {
          field_other_copy = clone(field_merged);
        } 
        else { 
          field_other_copy = clone(field_other);
        ...
        if (field_other.subfields && options.subfields) {
          ...
          Object.keys(options.subfields).forEach(function(code) {
            var subfield_options = options.subfields[code];

            var field_other_subfields = field_other.subfields.filter(function (subfield) {
              return subfield.code == code;
            });

            var field_other_copy_subfields = clone(field_other_subfields);

            if (field_other_copy_subfields.length == 0) {
              field_other_copy_subfields = [{code: code, value: ''}];
            }


            field_other_copy_subfields.forEach(function(field_other_copy_subfield) {
              var subfield_modifications = subfield_options.modifications || [];

              if (subfield_options.convertCode) field_other_copy_subfield.code = subfield_options.convertCode;
              if (subfield_options.replaceValue) field_other_copy_subfield.value = subfield_options.replaceValue;

              subfield_modifications.forEach(function(subfield_modifications_step) {
                if (stringOperations.hasOwnProperty(subfield_modifications_step.type)) {
                  var args = subfield_modifications_step.args;

                  field_other_copy_subfield.value = stringOperations[subfield_modifications_step.type](field_other_copy_subfield.value, subfield_modifications_step.args);
                } else {
            ...
            if (subfield_options.append) {
              ...
              if (field_other_copy_target_subfields.length > 1) {
                ...
                field_other_copy_target_subfield.value = field_other_copy_target_subfield.value + field_other_copy_subfields.reduce(function(str, subfield) {
                  return str + subfield.value;
                }, '');
              }
              else field_other_copy_subfields.forEach(function(subfield) {
                insertSubfield(field_other_copy, subfield);
              });
            } 
            else {
              ...
              field_other_copy_subfields.forEach(function(subfield) {
                insertSubfield(field_other_copy, subfield);
              });
            }
          });
        ...
        if (options.convertTag) {
          ...
          if (fields_merged.length > 0 && !options.keepExisting) {
            record_merged.fields.splice(record_merged.fields.indexOf(field_merged), 1);
          }
        }

        if (fields_merged.length == 0 || options.convertTag || options.keepExisting) {
    ...
    function insertSubfield (field, subfield) 
    {
      var code = subfield.code;

      var index_insert;

      field.subfields.some(function(subfield_field, index) {
        if (subfield_field.code > code) { 
          index_insert = index;
          return true;
        }
      });

      field.subfields.splice(index_insert === undefined ? field.subfields.length : index_insert, 0, subfield);
    }
