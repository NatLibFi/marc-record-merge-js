...
(function () {
  ...
  var Object = require('@natlibfi/es6-polyfills/lib/polyfills/object');
  var shim_array = require('@natlibfi/es6-shims/lib/shims/array');
  var MarcRecord = require('marc-record-js');
  var MarcRecordPunctuation = require('@natlibfi/melinda-marc-record-utils/dist/punctuation/index');
  ...
  function mergeFactory(configuration, options)
  {
    ...
    function applyPlugins(obj, name)
    {
      if (typeof options === 'object') {
        ...
        if (typeof options.actions === 'object') {
          Object.keys(options.actions).filter(function(key) {
            return typeof options.actions[key] === 'function';
          }).forEach(function(key) {
            actions[key] = options.actions[key];
          });
        }
        
        if (typeof options.comparators === 'object') {
          Object.keys(options.comparators).filter(function(key) {
            return typeof options.comparators[key] === 'function';
          }).forEach(function(key) {
            comparators[key] = options.comparators[key];
          });
        }
    ...
    return function(record_preferred, record_other, return_details)
    {
      ...
      var punctuationFixer;

      if (typeof configuration.fixPunctuation === 'object') {
        punctuationFixer = MarcRecordPunctuation.createRecordFixer(MarcRecordPunctuation.readPunctuationRulesFromJSON(configuration.fixPunctuation));
      }

      if (punctuationFixer) {
        record_merged.fields.forEach(function(field) {
          punctuationFixer(field);
        });
      }
