...
(function(root, factory) {
	...
	function Merger(config) {
			...
			other.fields.forEach(function(fieldInOther) {
				...
				if (fieldConfig !== undefined) {
					...
					try {
						...
						if (fieldConfig.action === 'controlfield') {
							actionControlfield(merged, fieldInOther, fieldConfig);
						}

						if (fieldConfig.action === 'selectBetter') {
							actionSelectBetter(merged, fieldInOther, fieldConfig);
						}
		...
		function actionControlfield(merged, fieldInOther, config) {

			var fieldsInMerged = merged.fields.filter(function(field) { return field.tag == fieldInOther.tag; });
			if (fieldsInMerged.length === 0) {
		...
		function actionSelectBetter(merged, fieldInOther, config) {
			config.options = config.options || {};
			
			var fieldsInMerged = merged.fields.filter(function(field) { return field.tag == fieldInOther.tag; });
			if (fieldsInMerged.length >= 2) {
				var invalidRecordError = new Error("selectBetter cannot be used if there are multiple fields of same tag.");
				invalidRecordError.name = "InvalidRecordError";
				throw invalidRecordError;
			}

			if (fieldsInMerged.length === 0) {

				if (config.options.requireFieldInBoth === true) {
					return;
				}
				fieldInOther.wasUsed = true;
				fieldInOther.fromOther = true;
				...
				if (config.options.onlyIfMissing === true) {
					return;
				}
				
				var fieldInMerged = fieldsInMerged[0];

				var fieldInMergedNorm = normalizeField(fieldInMerged);
				var fieldInOtherNorm =  normalizeField(fieldInOther);

				var comparator = subfieldEquality;
				
				if (config.options !== undefined) {
					if (config.options.comparator === "substring") {
						comparator = subfieldSubstringComparator;
					}
				}



				if (setsIdentical(fieldInMergedNorm.subfields, fieldInOtherNorm.subfields, comparator)) {

					var subfieldPairs = makePairs(fieldInMergedNorm.subfields, fieldInOtherNorm.subfields, comparator);

					var sizes = subfieldPairs.map(function(pair) {
						var l0 = pair[0].value.length;
						var l1 = pair[1].value.length;

						return [l0 > l1, l1 > l0];
					});

					var scores = sizes.reduce(function(memo, pair) { 
						if (pair[0]) memo[0]++;
						if (pair[1]) memo[1]++;
						return memo;
					}, [0,0]);
					...
					if (scores[0] >= scores[1]) {
						//Field in merged is considered better
					} else {
						var fieldCopy = clone(fieldInOther);
						fieldInOther.wasUsed = true;
						fieldCopy.fromOther = true;

						merged.fields.splice(merged.fields.indexOf(fieldInMerged), 1, fieldCopy);
					}

				}
				
				if (isProperSubset( fieldInMergedNorm.subfields, fieldInOtherNorm.subfields, comparator) ) {
					// use fieldInOther
					var fieldInOtherCopy = clone(fieldInOther);


					
					fieldInOther.wasUsed = true;
					fieldInOtherCopy.fromOther = true;

					merged.fields.splice(merged.fields.indexOf(fieldInMerged), 1, fieldInOtherCopy);

				} else {
					// use fieldInPreferred, which is already in place.
		...
		// This method will try to make a list of equal pairs from both subfield lists using equalityfunc
		function makePairs(subfields1, subfields2, equalityFunc) {
			if (subfields1.length !== subfields2.length) {
				return false;
			}

			var permutations1 = permute(subfields1);
			for (var i=0;i<permutations1.length;i++) {

				if (permutations1[i].every(subEq)) {
					
					return permutations1[i].map(pairWith(subfields2));
				}
			...
			function subEq(subfield, idx) {
				return equalityFunc(subfield, subfields2[idx]);
			}

			function pairWith(array) {
		...
		function permute(input) {
			...
			return (function permuteOne(input) {
		...
		function isProperSubset(subset, superset, equalityFunc) {
			return isSubset(subset, superset, equalityFunc) && !isSubset(superset, subset, equalityFunc);
		...
		function actionCopy(merged, fieldInOther, config) {
			...
			var fieldInPreferred = find(merged.fields, function(fieldFromPreferred) {
				return fieldsIdentical(fieldFromPreferred, fieldInOther, config);
			});

			if (fieldInPreferred === undefined) {
				fieldInPreferred = find(merged.fields, function(fieldFromPreferred) {
		...
		function fieldsIdentical(field1, field2, config) {
			var cfg = clone(config);
			cfg.options.mustBeIdentical = true;
			return fieldsSimilar(field1, field2, cfg);
		}
		...
		function subfieldSubstringComparator(subfield1, subfield2) {
			return subfield1.code == subfield2.code && (
				subfield1.value.indexOf(subfield2.value) != -1 || 
				subfield2.value.indexOf(subfield1.value) != -1);
		}
