...
(function(root, factory) {
	...
	if (typeof define === 'function' && define.amd) {
		define(['q','marc-record-js'], factory);
		...
		function actionCopy(merged, fieldInOther, config) {
			...
			if (fieldInPreferred === undefined) {
				...
				if (fieldInPreferred.subfields !== undefined &&
					...
					var preferredRemovedSubfields = getCompareWithoutSubfields(fieldInPreferred, config);
					var otherRemovedSubfields = getCompareWithoutSubfields(fieldInOther, config);
				...
				if ( isSubset(normalizedFieldInPreferred.subfields, normalizedFieldInOther.subfields,     subfieldEquality) &&
					...
					if (config.options.combine !== undefined) {
						combineSubfields('c', fieldInOther);
					}
					...
					if (config.options.combine !== undefined) {
						combineSubfields('c', fieldInPreferred);
					}
				}
