...
(function(root, factory) {
	...
	function Merger(config) {
		...
		function getConfigurationFor(tag) {
			...
			var matcherSpec = matcherArray.find(function(matcher) {
		...
		function actionCopy(merged, fieldInOther, config) {
			...
			var fieldInPreferred = merged.fields.find(function(fieldFromPreferred) {
			...
			if (config.options.compareWithout !== undefined) {
				config.options.compareWithout.forEach(function(filterCode) {
					[fieldInPreferred, fieldInOther].forEach(function(field) {
						...
						compareWithoutSubfields = compareWithoutSubfields.concat(cws);
					});
				});
			}

			compareWithoutSubfields = compareWithoutSubfields.reduce(function(memo, subfield) {
				var has = memo.find(subfieldEquality.bind(this, subfield));
				if (has === undefined) {
					memo.push(subfield);
				}
				return memo;
			...
			if (fieldInPreferred === undefined) {
				...
				if ( isSubset(normalizedFieldInPreferred.subfields, normalizedFieldInOther.subfields,     subfieldEquality) &&
					...
					fieldInOther.subfields = fieldInOther.subfields.concat(compareWithoutSubfields);
					fieldInOther.wasUsed = true;
					fieldInOther.fromOther = true;
