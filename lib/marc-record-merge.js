	...
	function Merger(config) {
		...
		function merge(record1, record2) {
			...
			other.fields.forEach(function(fieldInOther) {
				...
				if (config.fields[fieldInOther.tag] !== undefined) {


					if (config.fields[fieldInOther.tag].action === 'copy') {
						actionCopy(merged, fieldInOther);
					}
					if (config.fields[fieldInOther.tag].action === 'moveSubfields') {
						actionMoveSubfields(merged, fieldInOther, config.fields[fieldInOther.tag]);
					}
		...
		function actionMoveSubfields(merged, fieldInOther, config) {
			...
			var fieldInPreferred = merged.fields.find(fieldsIdenticalCheck(fieldInOther));
		...
		function actionCopy(merged, fieldInOther, config) {
			...
			var fieldInPreferred = merged.fields.find(fieldsSimilar.bind(this, fieldInOther));
			...
			if (fieldInPreferred === undefined) {
				...
				if (isSubset(fieldInPreferred.subfields, fieldInOther.subfields, subfieldEquality) &&
					!isSubset(fieldInOther.subfields, fieldInPreferred.subfields, subfieldEquality)) {
					// preferred records subfields are subset of other records fields.

					// this should probably be an option, since sometimes we always want to keep the preferred. maybe.
				
					merged.fields.splice(merged.fields.indexOf(fieldInPreferred), 1, fieldInOther);
